...
public interface ApplicationRepository extends CrudRepository<Application,Integer> {
    ...
    @Query(value = "SELECT distinct a.*, r.municipality FROM applications a JOIN registrations r ON a.registration = r.id WHERE r.municipality NOT IN (SELECT l.municipalityId FROM log_emails l WHERE l.action = 'createApplication' and sent_date > ?3 ) AND a.date < ?2 AND call_id = ?1 ORDER BY date ASC", nativeQuery = true)
