...
public class SupplierValidator {
    ...
    public static void validateSupplierNoBicAccount(SupplierDTO supplierDTO) throws Exception{
        if (supplierDTO.getStatus() != 0) {
            throw new Exception("Incorrect supplier status");
        }
        ...
        if ((Validator.isNull(supplierDTO.getAddress()) || supplierDTO.getAddress().trim().isEmpty())
                ...
                || (Validator.isNull(supplierDTO.getContactNumber()) || supplierDTO.getContactNumber().trim().isEmpty())
                || (Validator.isNull(supplierDTO.getContactPrefix()) || supplierDTO.getContactPrefix().trim().isEmpty())) {
            throw new Exception("Some field is empty!");
        }
