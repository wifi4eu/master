...
public interface VoucherSimulationRepository extends CrudRepository<VoucherSimulation, Integer> {
    ...
    @Query(value = "SELECT count(va) FROM VoucherAssignment va WHERE va.call.id =:callId AND va.status =:status")
    Integer checkIfSimulationExistByCallId(@Param("callId") Integer callId, @Param("status") Integer status);
}