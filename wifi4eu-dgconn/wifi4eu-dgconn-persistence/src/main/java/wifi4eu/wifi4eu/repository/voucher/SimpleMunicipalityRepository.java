...
public interface SimpleMunicipalityRepository extends CrudRepository<SimpleMunicipality, Integer> {
    ...
    @Query(value = "SELECT municipalities.lau FROM municipalities INNER JOIN registrations ON municipalities.id = registrations.municipality INNER JOIN applications ON registrations.id = applications.registration WHERE municipalities.country IS NOT NULL AND municipalities.name IS NOT NULL AND applications.id = ?1", nativeQuery = true)
    Integer findLauFromApplication(Integer applicationId);
