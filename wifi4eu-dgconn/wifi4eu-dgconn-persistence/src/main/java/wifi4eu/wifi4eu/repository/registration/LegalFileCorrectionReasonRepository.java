...
public interface LegalFileCorrectionReasonRepository extends CrudRepository<LegalFileCorrectionReason, Integer> {
    ...
    @Query(value = "SELECT COUNT(0) FROM legal_files_correction_reason WHERE request_correction IS NOT NULL AND (correction_reason IS NULL OR correction_reason = '') AND (request_correction_date IS NOT NULL AND request_correction_date > ?#{[0]})", nativeQuery = true)
