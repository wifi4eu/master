...
public interface RegistrationUsersRepository extends CrudRepository<RegistrationUsers, Integer> {
    ...
    @Query(value = "SELECT contact_email FROM registration_users where registration = (SELECT registration FROM applications where id = ?1)", nativeQuery = true)
    String findContactEmailFromApplicationId(int applicationId);

    @Query(value = "SELECT _user FROM registration_users where registration = (SELECT registration FROM applications where id = ?1)", nativeQuery = true)
