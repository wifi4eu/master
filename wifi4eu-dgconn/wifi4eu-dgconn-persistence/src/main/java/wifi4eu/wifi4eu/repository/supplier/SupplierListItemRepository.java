...
public interface SupplierListItemRepository extends CrudRepository<SupplierListItem,Integer> {
    ...
    @Query(value = "select sup.id, sup.name, sup.website, sup.vat, sup._status as status, (select count(1) from suppliers where (account_number = sup.account_number or vat = sup.vat) and _status != 1) as numberRegistrations from suppliers as sup WHERE sup.name like concat('%',?#{[0]},'%') ORDER BY name ASC OFFSET ?#{[1]} ROWS FETCH NEXT ?#{[2]} ROWS ONLY", nativeQuery = true)
    List<SupplierListItem> findSupplierListItemsOrderByNameAsc(String name, Integer offset, Integer count);

    @Query(value = "select sup.id, sup.name, sup.website, sup.vat, sup._status as status, (select count(1) from suppliers where (account_number = sup.account_number or vat = sup.vat) and _status != 1) as numberRegistrations from suppliers as sup WHERE sup.name like concat('%',?#{[0]},'%') ORDER BY name DESC OFFSET ?#{[1]} ROWS FETCH NEXT ?#{[2]} ROWS ONLY", nativeQuery = true)
    List<SupplierListItem> findSupplierListItemsOrderByNameDesc(String name, Integer offset, Integer count);

    @Query(value = "select sup.id, sup.name, sup.website, sup.vat, sup._status as status, (select count(1) from suppliers where (account_number = sup.account_number or vat = sup.vat) and _status != 1) as numberRegistrations from suppliers as sup WHERE sup.name like concat('%',?#{[0]},'%') ORDER BY website ASC OFFSET ?#{[1]} ROWS FETCH NEXT ?#{[2]} ROWS ONLY", nativeQuery = true)
    List<SupplierListItem> findSupplierListItemsOrderByWebsiteAsc(String name, Integer offset, Integer count);

    @Query(value = "select sup.id, sup.name, sup.website, sup.vat, sup._status as status, (select count(1) from suppliers where (account_number = sup.account_number or vat = sup.vat) and _status != 1) as numberRegistrations from suppliers as sup WHERE sup.name like concat('%',?#{[0]},'%') ORDER BY website DESC OFFSET ?#{[1]} ROWS FETCH NEXT ?#{[2]} ROWS ONLY", nativeQuery = true)
    List<SupplierListItem> findSupplierListItemsOrderByWebsiteDesc(String name, Integer offset, Integer count);

    @Query(value = "select sup.id, sup.name, sup.website, sup.vat, sup._status as status, (select count(1) from suppliers where (account_number = sup.account_number or vat = sup.vat) and _status != 1) as numberRegistrations from suppliers as sup WHERE sup.name like concat('%',?#{[0]},'%') ORDER BY vat ASC OFFSET ?#{[1]} ROWS FETCH NEXT ?#{[2]} ROWS ONLY", nativeQuery = true)
    ...
    @Query(value = "select sup.id, sup.name, sup.website, sup.vat, sup._status as status, (select count(1) from suppliers where (account_number = sup.account_number or vat = sup.vat) and _status != 1) as numberRegistrations from suppliers as sup WHERE sup.name like concat('%',?#{[0]},'%') ORDER BY status ASC OFFSET ?#{[1]} ROWS FETCH NEXT ?#{[2]} ROWS ONLY", nativeQuery = true)
