...
public class ApplicationInvalidateReasonResource {
    ...
    public List<ApplicationInvalidateReasonDTO> getInvalidateReasonsByApplication(@PathVariable("applicationId") Integer applicationID, HttpServletResponse response) throws IOException {
        ...
        try{
            ...
            _log.error("ECAS Username: " + userConnected.getEcasUsername() + "- You have no permissions to retrieve invalidated reasons by application ID: " + applicationID, adex.getMessage());
    ...
    @ResponseBody
    public ResponseEntity<List<ApplicationInvalidateReasonDTO>> invalidateApplicationWithReason(@RequestBody InvalidReasonViewDTO invalidReasonViewDTO, HttpServletResponse response, HttpServletRequest request) throws IOException {
        UserContext userContext = UserHolder.getUser();
        UserDTO userConnected = userService.getUserByUserContext(userContext);
