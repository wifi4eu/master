...
left join V_LOC_COMMITMENT_HEADER@ABACBUDT_SHARED bc on request.L_LOC_OBJ_FK = (bc.loc_sys_cd || '.' || bc.run_id)
left join v_o_gen_object_References@ABACBUDT_SHARED oj on oj.loc_sys_cd = 'WIF' and ('WIF.' || substr(oj.loc_obj_Foreign_id, 5, instr(oj.loc_obj_Foreign_id, '.')+1)) = request.L_LOC_OBJ_FK
left join V_O_LOG_ERRORS@ABACBUDT_SHARED err ON err.loc_sys_cd = 'WIF' and request.L_LOC_OBJ_FK = (err.loc_sys_cd || '.' || err.batch_id) and err.msg_tp_cd <> 'I'
...
from wif_abac_request_process request
left join V_LOC_LCM_HEADER@ABACBUDT_SHARED lc on request.L_LOC_OBJ_FK = (lc.loc_sys_cd || '.' || lc.run_id)
left join v_o_gen_object_References@ABACBUDT_SHARED oj on oj.loc_obj_Foreign_id = request.L_LOC_OBJ_FK
left join V_O_LOG_ERRORS@ABACBUDT_SHARED err ON err.loc_sys_cd = 'WIF' and request.L_LOC_OBJ_FK = (err.loc_sys_cd || '.' || err.batch_id) and err.msg_tp_cd <> 'I'
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "WIFI4EU_ABAC"."SEQ_ABAC_RUN_ID"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 22000 CACHE 20 NOORDER  NOCYCLE ;
--------------------------------------------------------
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "WIFI4EU_ABAC"."SEQ_DOCUMENT"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 161 CACHE 20 NOORDER  NOCYCLE ;
--------------------------------------------------------
--  DDL for Sequence SEQ_IMPORT_LOG
--------------------------------------------------------

   CREATE SEQUENCE  "WIFI4EU_ABAC"."SEQ_IMPORT_LOG"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 81 CACHE 20 NOORDER  NOCYCLE ;
--------------------------------------------------------
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "WIFI4EU_ABAC"."SEQ_LEGAL_ENTITY"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 284 NOCACHE  NOORDER  NOCYCLE ;
--------------------------------------------------------
--  DDL for Sequence SEQ_NOTIFICATION
--------------------------------------------------------

   CREATE SEQUENCE  "WIFI4EU_ABAC"."SEQ_NOTIFICATION"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 101 CACHE 20 NOORDER  NOCYCLE ;
--------------------------------------------------------
--  DDL for Sequence SEQ_WIF_ABAC_STATUS
--------------------------------------------------------

   CREATE SEQUENCE  "WIFI4EU_ABAC"."SEQ_WIF_ABAC_STATUS"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 341 CACHE 20 NOORDER  NOCYCLE ;
--------------------------------------------------------
...
BEGIN
  ...
  FOR legalCommitment in (
    ...
    where lc.WF_STATUS = 'COUNTERSIGNED'
      ...
      -- STEP 1 - Creation of the Commitment header
      Insert into V_LOC_LCM_HEADER@ABACBUDT_SHARED
     ...
     Values
       (l_run_id, 10, l_loc_sys_cd, 'ABACBUDT', 'LCM',
      ...
      Insert into V_LOC_LCM_CONTRACT_MGMT@ABACBUDT_SHARED
      ...
      Values
       (l_run_id, 20, l_loc_sys_cd, 'ABACBUDT', 'LCM', 'LCH', 'CREATE', 'LEGAL COMMITMENT CONTRACT MGMT', l_loc_obj_foreign_id, 'N', legalCommitment.LE_FEL_ID, 'BC', legalCommitment.COMMITMENT_L2_AMOUNT);

      -- STEP 3 - Registration of responsible person
      Insert into V_LOC_GEN_ORE_RSP_USERS@ABACBUDT_SHARED
        (RUN_ID, ROW_ID, LOC_SYS_CD, DESTINATION, TRANS_AREA_CD, TRANS_TP_CD, TRANS_ACTION_CD, TABLE_ALIAS, LOC_OBJ_FOREIGN_ID, PROCESSED, PERSON_ID)
      Values
        (l_run_id, 40, l_loc_sys_cd, 'ABACBUDT', 'LCM', 'LCH', 'CREATE', 'LEGAL COMMITMENT RESP USER', l_loc_obj_foreign_id, 'N', 'BILLEWO');


      -- STEP 4 - Registration of the related Budget Commitment
      Insert into V_LOC_LCM_HEADER_COM_LINK@ABACBUDT_SHARED
      ...
      Values
         (l_run_id, 110, l_loc_sys_cd, 'ABACBUDT', 'LCM', 'LCH', 'CREATE', 'LEGAL COMMITMENT HEADER LINK', l_loc_obj_foreign_id, 'N', legalCommitment.COMMITMENT_L2_KEY);
     ...
     Values
       (l_run_id, 120, l_loc_sys_cd, 'ABACBUDT', 'LCM', 'LCH', 'CREATE', 'VISA', l_loc_obj_foreign_id,
  ...
  loop
      ...
      if (request.bc_status <> request.abac_status) then
        ...
        for position in (
                        ...
                        join V_LOC_COMMITMENT_POSITION@ABACBUDT_SHARED abac_pos on wif_pos.commitment_l2_position = abac_pos.commit_pos_line_no and abac_pos.loc_obj_foreign_id = request.l_loc_obj_fk
