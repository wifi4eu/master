...
//    AND r.allFilesFlag = 1
    ...
    @Query(value = "SELECT a.* FROM applications a JOIN registration_users ru ON a.registration = ru.registration JOIN users u ON ru._user = u.id WHERE u.ecas_email NOT IN (SELECT l.toAddress FROM log_emails l WHERE l.action = 'createApplication') AND a.date < ?1 order by date asc", nativeQuery = true)
