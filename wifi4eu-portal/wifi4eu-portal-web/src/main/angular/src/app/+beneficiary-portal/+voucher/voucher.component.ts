...
export class VoucherComponent {
  ...
  constructor(
    ...
    private azurequeueApi: AzurequeueApi,
    private sharedService: SharedService,
    private http: Http
  ) {
    let storedUser = this.localStorage.get("user");
    this.user = storedUser ? JSON.parse(storedUser.toString()) : null;
    let storedRegistrations = this.localStorage.get("registrationQueue")
      ? JSON.parse(this.localStorage.get("registrationQueue").toString())
      : null;
    this.storedRegistrationQueues = storedRegistrations
      ? storedRegistrations
      : [];
    // Check if there are Calls
    if (this.user != null) {
      this.registrationApi
        .getRegistrationsByUserId(this.user.id)
        .subscribe((registrations: RegistrationDTOBase[]) => {
          ...
          if (registrations.length < 2) {
            ...
            this.mayorApi
              .getMayorByMunicipalityId(registrations[0].municipalityId)
              .subscribe(
                (mayor: MayorDTOBase) => {
                  this.mayor = mayor;
                  // HERE WE CHECK IF ITS REPRESENTATIVE OR NOT
                  if (
                    this.mayor.name == this.user.name &&
                    this.mayor.surname == this.user.surname
                  ) {
                    this.isMayor = true;
                  } else {
                    this.isMayor = false;
    ...
    this.callApi.allCalls().subscribe(
      (calls: CallDTOBase[]) => {
        ...
        for (let registration of registrations) {
          this.municipalityApi
            .getMunicipalityById(registration.municipalityId)
            .subscribe((municipality: MunicipalityDTOBase) => {
              if (municipality != null) {
                this.mayorApi
                  .getMayorByMunicipalityId(municipality.id)
                  .subscribe((mayor: MayorDTOBase) => {
                    if (this.currentCall) {
                      this.applicationApi
                        .getApplicationByCallIdAndRegistrationId(
                          this.currentCall.id,
                          registration.id
                        )
                        .subscribe((application: ApplicationDTOBase) => {
                          ...
                          var res = this.storedRegistrationQueues.filter(
                            queue => {
                          ...
                          if (
                            this.currentCall.startDate - new Date().getTime() <=
                            0
                          ) {
                          ...
                          if (
                            this.applications.length == registrations.length
                          ) {
                            ...
                            this.disableQueuing.forEach(
                              (disableQueuingEl, index) => {
                                this.loadingButtons[index] = disableQueuingEl
                                  ? true
                                  : false;
                                if (disableQueuingEl != null)
                                  newStoredRegistrationQueues.push(
                                    disableQueuingEl
                                  );
                              }
                            );
  ...
  private goToDocuments(registrationNumber: number) {
    this.router.navigate(
      [
        "../additional-info/",
        this.registrations[registrationNumber].municipalityId
      ],
  ...
  private applyForVoucher(registrationNumber: number, event) {
    ...
    if (startCallDate <= actualDateTime) {
      if (!this.loadingButtons[registrationNumber]) {
        ...
        this.http
          .post(this.rabbitmqURI, { message: m })
          .toPromise()
          .then(
            () => {
  ...
  private checkForDocuments() {
    if (this.isMayor) {
      this.docsOpen[0] =
        this.registrations[0].legalFile1 != null &&
        this.registrations[0].legalFile4 == null &&
        this.registrations[0].legalFile2 == null &&
        this.registrations[0].legalFile3 != null;
      ...
      for (let i = 0; i < this.registrations.length; i++) {
        this.docsOpen[i] =
          this.registrations[i].legalFile1 != null &&
          this.registrations[i].legalFile3 != null;
        if (this.docsOpen[i]) {
