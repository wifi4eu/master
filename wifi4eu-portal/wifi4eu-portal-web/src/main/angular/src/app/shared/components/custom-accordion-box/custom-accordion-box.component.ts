    ...
    template: `
        <div class="box accordion-box" [class.expanded]="isExpanded">
            <div class="header" style="cursor: default;">
                <span *ngIf="closable" class="closable"
                      [style.padding]="rightSide ? '0.65rem 0 0 1rem' : '0.65rem 1rem 0 0'"
                      style="cursor: pointer;" [style.float]="rightSide ? 'left' : 'right'" (click)="close()"><i
                        class="fa fa-times"></i></span>
                <span class="title" [style.padding]="rightSide ? '0.65rem 1rem 0 0' : '0.65rem 0 0 1rem'"
                      style="cursor: pointer;" [style.float]="rightSide ? 'right' : 'left'"
                      (click)="toggle()">{{ label | translate}}
