...
export class DgConnVoucherComponent {
    ...
    constructor(private http: Http, private lauApi: LauApi, private nutsApi: NutsApi, private uxService: UxService, private translate: TranslateService) {
        this.mapTableData = [];
        ...
        if (event.keyCode > 64 && event.keyCode < 91) {
            if (typeof this.nutsDTO === "string") {
                ...
                for (let i = 0; i < nuts.length; i++) {
                    let nut = nuts[i];
                    if (nut.label.toLowerCase().indexOf(name.toLowerCase()) == 0) {
                        this.nutsDTO = nut;
                    }
                }
            }
    ...
    filterCountries(query, nuts: NutsDTOBase[]) {
        ...
        for (let i = 0; i < nuts.length; i++) {
            ...
            if (nut.label.indexOf(query.toLowerCase()) == 0) {
                nut.label = nut.label.charAt(0).toUpperCase() + nut.label.slice(1);
